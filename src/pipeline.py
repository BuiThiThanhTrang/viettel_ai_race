...
class MedicalNEREncoder(MedicalNERBase):
    def __init__(self, model_name="TrangBui/BamiBERT-ViMedNER-Finetuned-398"):
    ...
    def extract(self, text):
        ...
        def segment_text_preserve_length(t):
            from pyvi import ViTokenizer
            tokenized_text = ViTokenizer.tokenize(t)
            ...
            for token in tokens:
                if '_' in token:
                    ...
                    pos = t.find(original_token, search_idx)
                    if pos != -1:
        ...
        processed_text = segment_text_preserve_length(text)
...
class AssertionClassifierEncoder(AssertionClassifierBase):
    def __init__(self, model_path="..\data\output\model_assertion_output"):
